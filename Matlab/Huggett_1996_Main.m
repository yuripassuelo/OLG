...
% Define discretização de choques
...
plot( linspace( cS.age1, cS.ageLast , cS.ageLast - cS.age1 + 1 ) , paramS.ageEffV, 'LineWidth', 1.5 )
xlim([20 98])
xlabel('Idade','Interpreter', 'latex');
ylabel('Produtividade','Interpreter', 'latex');

fig=gcf;
fig.Position(3:4)=[500,300];

exportgraphics(gcf,'Plots/Plot_5.pdf','ContentType','vector')

% Aproximacao Linear do Hugget - Alternativa
...
% Dependency Ratio (Razoes de Dependencia)
...
writetable(Tabela_Resumo,'Results/ResultadosPrincipais_1.txt')
...
writetable(Tabela_Secundaria,'Results/ResultadosSecundarios_1.txt')
...
exportgraphics(gcf,'Plots/Plot_Geral_1.pdf','ContentType','vector')
...
exportgraphics(gcf,'Plots/Plot_Geral_1_v2.pdf','ContentType','vector')
...
exportgraphics(gcf,'Plots/Plot_Geral_1_v3.pdf','ContentType','vector')
...
exportgraphics(gcf,'Plots/Plot_Geral_1_v4.pdf','ContentType','vector')

% Analise Riqueza e Gini Por idade

vec_w_meanM1 = zeros( [1 79]);
vec_w_medM1  = zeros( [1 79]);
vec_w_10M1   = zeros( [1 79]);
vec_w_25M1   = zeros( [1 79]);

vec_w_meanM2 = zeros( [1 79]);
vec_w_medM2  = zeros( [1 79]);
vec_w_10M2   = zeros( [1 79]);
vec_w_25M2   = zeros( [1 79]);

vec_w_meanM3 = zeros( [1 79]);
vec_w_medM3  = zeros( [1 79]);
vec_w_10M3   = zeros( [1 79]);
vec_w_25M3   = zeros( [1 79]);

for i = 1:79

    vec_w_meanM1(1,i) = mean( kprimeHistM1(:,i) )  ;
    vec_w_medM1(1,i)  = median( kprimeHistM1(:,i) )  ;
    vec_w_10M1(1,i)   = quantile( kprimeHistM1(:,i), .1 )  ;
    vec_w_25M1(1,i)   = quantile( kprimeHistM1(:,i), .25 )  ;

    vec_w_meanM2(1,i) = mean( kprimeHistM2(:,i) )  ;
    vec_w_medM2(1,i)  = median( kprimeHistM2(:,i) )  ;
    vec_w_10M2(1,i)   = quantile( kprimeHistM2(:,i), .1 )  ;
    vec_w_25M2(1,i)   = quantile( kprimeHistM2(:,i), .25 )  ;

    vec_w_meanM3(1,i) = mean( kprimeHistM3(:,i) )  ;
    vec_w_medM3(1,i)  = median( kprimeHistM3(:,i) )  ;
    vec_w_10M3(1,i)   = quantile( kprimeHistM3(:,i), .1 )  ;
    vec_w_25M3(1,i)   = quantile( kprimeHistM3(:,i), .25 )  ;
...
% Gini por Idade (Distribuição Intra Idade)

gini_vecM1 = zeros([ 1 79 ]);
gini_vecM2 = zeros([ 1 79 ]);
gini_vecM3 = zeros([ 1 79 ]);

for i = 1:79

    gini_vecM1(1,i) = ComputeGini( kprimeHistM1(:,i)' );
    gini_vecM2(1,i) = ComputeGini( kprimeHistM2(:,i)' );
    gini_vecM3(1,i) = ComputeGini( kprimeHistM3(:,i)' );

end

ages = linspace( 20, 98, 79);

plot( ages, gini_vecM1 )
hold on
plot( ages, gini_vecM2 )
plot( ages, gini_vecM3 )
hold off
title( "$\sigma = 1.5$", "Interpreter", "latex")
xlim([20 92])
xlabel( "Idade", "Interpreter", "latex" )
ylabel( "GINI", "Interpreter", "latex")
legend( "$\eta=-0.012$", "$\eta=0.0$", "$\eta=0.012$", "Interpreter", "latex" )

% Exporta Dados Relacionados ao Kprime


writematrix( kprimeHistM1 , 'Results/Data/Matriz_n1_1.dat')
writematrix( kprimeHistM2 , 'Results/Data/Matriz_n2_1.dat')
writematrix( kprimeHistM3 , 'Results/Data/Matriz_n3_1.dat')
